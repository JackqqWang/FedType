...
from utils_conformal_text import validate, get_logits_targets, sort_sum
from options_conformal import args_parser
...
class ConformalModel(nn.Module):
    def __init__(self, args, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        super(ConformalModel, self).__init__()
        self.model = model 
        self.alpha = alpha
        self.T = torch.Tensor([1.3]) #initialize (1.3 is usually a good value)
        self.T, calib_logits = platt(args, self, calib_loader)
    ...
    def forward_text(self, *args, randomized=None, allow_zero_sets=None, databatch=None):
        if randomized == None:
            randomized = self.randomized
        if allow_zero_sets == None:
            allow_zero_sets = self.allow_zero_sets

        inputs = databatch['input_ids'].to(device)
        attention_mask = databatch['attention_mask'].to(device)
...
class small_ConformalModel(nn.Module):
    def __init__(self, args, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        super(small_ConformalModel, self).__init__()
        self.model = model 
        self.alpha = alpha
        self.T = torch.Tensor([1.3]) #initialize (1.3 is usually a good value)
        self.T, calib_logits = platt(args, self, calib_loader)
...
class large_ConformalModel(nn.Module):
    def __init__(self, args, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        super(large_ConformalModel, self).__init__()
        self.model = model 
        self.alpha = alpha
        self.T = torch.Tensor([1.3]) #initialize (1.3 is usually a good value)
        self.T, calib_logits = platt(args, self, calib_loader)
...
def platt(args, cmodel, calib_loader, max_iters=10, lr=0.01, epsilon=0.01):
    print("Begin Platt scaling.")
    # Save logits so don't need to double compute them
    logits_dataset = get_logits_targets(cmodel.model, calib_loader, args.num_classes, args)
...
def platt_logits(cmodel, calib_loader, max_iters=10, lr=0.01, epsilon=0.01, args = args):
