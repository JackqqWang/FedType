...
def my_validate(val_loader, model):
    with torch.no_grad():
...
def get_logits_targets(model, loader, num_classes, args):
    ...
    with torch.no_grad():
        for x, targets in loader:
            if args.dataset == 'fmnist':
                x = x.repeat(1,3,1,1)
            # x = x.repeat(1,3,1,1) # for grayscale images
            # print(x.shape)
...
def get_logits_dataset(modelname, datasetname, datasetpath, cache=str(pathlib.Path(__file__).parent.absolute()) + '/experiments/.cache/'):
    ...
    dataset_logits = get_logits_targets(model, loader, len(dataset.classes),args)
