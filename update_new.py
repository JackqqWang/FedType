...
def uni_student_ranking_loss(student_logits, teacher_logits, top_k):
    ...
    return batch_accumulator_loss/student_logits.shape[0]/len(top_k)
...
class LocalUpdate(object):
    ...
    def update_weights(self,global_round):
        ...
        # old large training acc
        large_train_acc_old = None
        consensus_hyper = 0.1
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                # get sample weights using small_cmodel
                with torch.no_grad():
                ...
                predicted_labels = torch.argmax(large_model_logits, dim=1)
                large_train_acc_new += (predicted_labels == labels).sum().item()
                ...
                # consensus_hyper = cal_sim_set(my_small_S, my_large_S)
                    
                consensus_hyper = cal_consensus(my_small_S, my_large_S)
  
                if self.args.verbose and (batch_idx % 10 == 0):

                    print("| Global Round : {} | Local Epoch : {} | batch {}, consensus_score is {}".format(global_round, iter, batch_idx, consensus_hyper))
                ...
                total_large_loss = ce_large_loss + consensus_hyper * rkd_loss
            ...
            large_train_acc_new /= len(self.trainloader.dataset)
            ...
            # update lambda
            change_train_acc = large_train_acc_new - large_train_acc_old if large_train_acc_old is not None else large_train_acc_new
            cur_lambda = cal_lambda(change_train_acc,args=self.args)
            large_train_acc_old = large_train_acc_new
        ...
        small_cmodel = small_ConformalModel(self.args, self.small_model, self.trainloader,\
                                             alpha=0.1, kreg=args.small_kreg, lamda=cur_lambda, lamda_criterion='size')
        small_cmodel.eval()
